	...
	flushClose(size) {
		if (this.closed) {
			...
			if (size > 1) {
				...
				if (trim)
					delimMin = delimMin.slice(
						0,
						delimMin.length - trim[0].length
					)
	...
	renderInline(parent) {
		...
		const progress = (node, _, index) => {
			...
			if (
				...
				const [_, lead, inner, trail] = /^(\s*)(.*?)(\s*)$/m.exec(
					node.text
				)
				leading += lead
				trailing = trail
			...
			while (keep < active.length)
				this.text(
					this.markString(active.pop(), false, parent, index),
					false
				)
